...
export const createVersion = async (req, res, next) => {
    ...
    try {
        const result = await versionService.createVersion(projectId, version, req.user.id, changes);
...
export const getVersions = async (req, res, next) => {
    ...
    try {
        const versions = await versionService.getProjectVersions(projectId, req.user.id);
        res.json(versions);
    }
    catch (error) {
        next(error);
